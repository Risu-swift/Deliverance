		...
		where TComponent : ListViewItem
	{
		...
		protected class ListViewTypeFixed : ListViewTypeBase
		{
			...
			public override float CalculateTopFillerSize()
			{
				...
				var size = Visible.FirstVisible * GetItemSize();
			...
			{
				var window = Owner.GetScrollValue() + Owner.GetScrollRectSize();
			...
			public override float GetItemPosition(int index)
			{
				...
				return (block_index * GetItemSize()) + Owner.LayoutBridge.GetMargin() - spacing;
			}
			...
			{
				return GetItemPosition(index) - (Owner.GetScrollRectSize() / 2) + (GetItemSize() / 2);
			}
			...
			{
				return GetItemPosition(index) + GetItemSize() + Owner.LayoutBridge.GetMargin() - Owner.LayoutBridge.GetSpacing() - Owner.GetScrollRectSize();
			}
