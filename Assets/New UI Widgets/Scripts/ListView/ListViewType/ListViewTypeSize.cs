		...
		where TComponent : ListViewItem
	{
		...
		protected class ListViewTypeSize : ListViewTypeBase
		{
			...
			protected virtual int CalculateMaxVisibleItems(ObservableList<TItem> items)
			{
				...
				var size = Owner.GetScrollRectSize();
			...
			{
				return GetItemsSize(0, Visible.FirstVisible);
			}
			...
			{
				var last_visible_index = GetIndexAtPosition(Owner.GetScrollValue() + Owner.GetScrollRectSize());
			...
			public override float GetItemPosition(int index)
			{
				...
				return size + (Owner.LayoutBridge.GetSpacing() * (index - 1)) + Owner.LayoutBridge.GetMargin();
			}
			...
			{
				return GetItemPosition(index) - (Owner.GetScrollRectSize() / 2) + (GetItemSize(index) / 2);
			}
			...
			{
				return GetItemPosition(index) + GetItemSize(index) + Owner.LayoutBridge.GetMargin() - Owner.LayoutBridge.GetSpacing() - Owner.GetScrollRectSize();
			}
