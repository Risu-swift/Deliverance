namespace EasyLayoutNS
{
	...
	public class EasyLayoutGrid : EasyLayoutCompactOrGrid
	{
		...
		int GetMaxColumnsCount(List<LayoutElementInfo> elements, int maxColumns)
		{
		...
		/// </summary>
		/// <param name="elements">User interface elements.</param>
		void GroupFlexible(List<LayoutElementInfo> elements)
		{
			int max_columns = 999999;
			while (true)
			{
				var new_max_columns = GetMaxColumnsCount(elements, max_columns);
			...
			{
				GroupByColumnsHorizontal(elements, max_columns);
			}
			else
			{
				GroupByRowsVertical(elements, max_columns);
			}
		}
